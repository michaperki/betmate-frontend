      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '16px 18px', borderBottom: '1px solid rgb(var(--text-primary-rgb) / 0.06)' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
            <div style={{ width: 36, height: 36, borderRadius: 10, background: 'rgb(var(--error-rgb) / 0.15)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>🏧</div>
            <div>
        ...
        <div style={{ padding: 18, overflowY: 'auto' }}>
          {step === 'amount' && (
            <>
                  ...
                  {presets.map(p => (
                    <button
                      ...
                      style={{
                        padding: '10px 8px',
                        borderRadius: 10,
                        background: amount === p && !custom ? 'rgb(var(--error-rgb) / 0.18)' : 'rgb(var(--text-primary-rgb) / 0.05)',
                        border: amount === p && !custom ? '1px solid rgb(var(--error-rgb) / 0.35)' : '1px solid var(--card-border)',
                        color: amount === p && !custom ? 'var(--error)' : 'var(--text-primary)',
                        fontWeight: 700,
                        cursor: 'pointer'
                      }}
                    >${p}</button>
              ...
              <div>
                <div style={{ position: 'relative' }}>
                  ...
                  <input
                    ...
                    style={{ width: '100%', padding: '10px 12px 10px 28px', borderRadius: 10, background: 'var(--input-bg)', border: custom ? '1px solid rgb(var(--error-rgb) / 0.35)' : '1px solid var(--input-border)', color: 'var(--input-text)', fontFamily: 'inherit' }}
                  />
                </div>
              ...
              <div style={{ display: 'flex', gap: 10, marginTop: 16 }}>
                ...
                <button onClick={onWithdraw} disabled={amount < 10} style={{ flex: 1, padding: 12, borderRadius: 10, background: amount >= 10 ? 'var(--error)' : 'rgb(var(--error-rgb) / 0.14)', border: 'none', color: amount >= 10 ? 'var(--text-inverse)' : 'var(--text-secondary)', fontWeight: 800, cursor: amount >= 10 ? 'pointer' : 'not-allowed' }}>Withdraw</button>
              </div>
            </>
          )}
          ...
          {step === 'processing' && (
            <div style={{ textAlign: 'center', padding: '30px 0' }}>
              <div style={{ width: 56, height: 56, margin: '0 auto 16px', borderRadius: '50%', border: '3px solid rgb(var(--error-rgb) / 0.15)', borderTopColor: 'var(--error)', animation: 'spin 1s linear infinite' }} />
          ...
          {step === 'success' && (
            <div style={{ textAlign: 'center', padding: '26px 0' }}>
              <div style={{ width: 72, height: 72, margin: '0 auto 16px', borderRadius: '50%', background: 'rgb(var(--error-rgb) / 0.15)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 34 }}>✅</div>
              ...
              <div style={{ display: 'flex', gap: 10 }}>
                <button onClick={onClose} style={{ flex: 1, padding: 12, borderRadius: 10, background: 'var(--error)', border: 'none', color: 'var(--text-inverse)', fontWeight: 800, cursor: 'pointer' }}>Close</button>
                <button onClick={() => setStep('amount')} style={{ flex: 1, padding: 12, borderRadius: 10, background: 'transparent', border: '1px solid var(--border-primary)', color: 'var(--text-primary)', cursor: 'pointer' }}>New Withdrawal</button>
              </div>
            </div>
          )}
        </div>
