...
const ReportIssue: React.FC<ReportIssueProps> = ({ isOpen, onClose }) => {
  ...
  return createPortal(
    <div role="dialog" aria-modal="true" aria-labelledby="issue-modal-title" style={{
      ...
      <div style={{
        position: 'relative', width: 'min(560px, 92vw)', background: 'linear-gradient(180deg, #1a1a24 0%, #12121a 100%)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 16, boxShadow: '0 20px 60px rgba(0,0,0,0.5)', color: '#e8e8e8', overflow: 'hidden',
      }}>
        <div style={{
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            <div style={{
              width: 32, height: 32, borderRadius: 8, background: 'rgb(var(--mode-accent-rgb) / 0.15)', display: 'flex', alignItems: 'center', justifyContent: 'center',
            }}>🐞</div>
            <div>
        ...
        <div style={{ padding: 16 }}>
          ...
          <textarea value={text} onChange={(e) => setText(e.target.value)} rows={5} placeholder="What happened? What did you expect?" style={{
            width: '100%', background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.15)', color: '#e8e8e8', borderRadius: 8, padding: 10, fontFamily: 'inherit',
          }} />
          <div style={{
            display: 'flex', gap: 10, justifyContent: 'flex-end', marginTop: 12,
          }}>
            <button onClick={onClose} style={{
              padding: '8px 12px', borderRadius: 8, background: 'transparent', border: '1px solid rgba(255,255,255,0.2)', color: '#e8e8e8',
            }}>Cancel</button>
            <button onClick={submit} disabled={submitting || text.trim().length < 3} style={{
              padding: '8px 12px', borderRadius: 8, background: 'linear-gradient(135deg, var(--mode-accent) 0%, var(--mode-accent-strong) 100%)', border: 'none', color: '#000', fontWeight: 800, opacity: (submitting || text.trim().length < 3) ? 0.6 : 1,
            }}>{success ? 'Sent!' : (submitting ? 'Sending…' : 'Send')}</button>
          </div>
        </div>
      </div>
    </div>,
