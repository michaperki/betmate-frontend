    ...
    <div className="admin-content">
      <div className="admin-card" style={{ marginTop: 0 }}>
        ...
        <Toolbar
          ...
          right={(
            <>
              <a href="/admin/users/search" style={{ color: '#9ca3af', textDecoration: 'none' }}>← Back to search</a>
      ...
      <div className="admin-card" style={{ marginTop: 16 }}>
        <DataTable
          ...
          columns={[
            { key: 'created_at', header: 'Date', sort: (a: any, b: any) => new Date(a.created_at).getTime() - new Date(b.created_at).getTime(), render: (r: any) => new Date(r.created_at).toLocaleString() },
            { key: 'currency', header: 'Currency', sort: true },
            { key: 'amount', header: 'Amount', align: 'right', sort: true, render: (r: any) => (r.currency === 'USDT' ? `$${Number(r.amount || 0).toFixed(2)}` : Number(r.amount || 0)) },
            { key: 'reason', header: 'Reason' },
            { key: 'reference', header: 'Ref', render: (r: any) => (r.reference_type ? `${r.reference_type}:${r.reference_id || ''}` : (r.reference_id || '—')) },
          ]}
        />
      </div>
    </div>
