...
const HelpFAQ: React.FC<{ isOpen: boolean; onClose: () => void } & { category?: string }> = ({ isOpen, onClose }) => {
  ...
  return createPortal(
    <div role="dialog" aria-modal="true" aria-label="Help and FAQ" style={{ position: 'fixed', inset: 0, zIndex: 10000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16 }}>
      ...
      <div style={{ position: 'relative', width: 'min(660px, 94vw)', maxHeight: '90vh', background: 'linear-gradient(180deg, #1a1a24 0%, #12121a 100%)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 16, boxShadow: '0 20px 60px rgba(0,0,0,0.5)', color: '#e8e8e8', overflow: 'auto' }}>
