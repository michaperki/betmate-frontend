...
const DrawOutcomeCard: React.FC<DrawOutcomeCardProps> = ({ disabled, odds, onPlace }) => {
  const { mode } = useMode();
  ...
  return (
    <div style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 12, padding: 16 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 10 }}>
        ...
        >
          <span style={{ fontSize: 12, fontWeight: 600, color: '#1a1a24' }}>White Wins</span>
          <span style={{ fontSize: 16, fontWeight: 700, color: '#1a1a24' }}>x{multipliers.white ? multipliers.white.toFixed(2) : '—'}</span>
        </button>
        ...
        >
          <span style={{ fontSize: 12, fontWeight: 600, color: '#e8e8e8' }}>Draw</span>
          <span style={{ fontSize: 16, fontWeight: 700, color: '#fbbf24' }}>x{multipliers.draw ? multipliers.draw.toFixed(2) : '—'}</span>
        </button>
        ...
        >
          <span style={{ fontSize: 12, fontWeight: 600, color: '#e8e8e8' }}>Black Wins</span>
          <span style={{ fontSize: 16, fontWeight: 700, color: '#22c55e' }}>x{multipliers.black ? multipliers.black.toFixed(2) : '—'}</span>
        </button>
      </div>
