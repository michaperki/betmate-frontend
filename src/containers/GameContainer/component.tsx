...
const GameContainer: React.FC = () => {
  ...
  const handleMoveBet = useCallback((move: string, index: number) => {
    ...
    setMoveOptions((prev) => prev.map((m, i) => (i === index ? { ...m, status: 'loading' } : m)));
  ...
  return (
    <div className={`new-game-page mode-${mode}`}>
      <div style={{
        position: 'fixed', top: '10%', left: '20%', width: '500px', height: '500px', background: 'radial-gradient(circle, rgb(var(--mode-accent-rgb) / 0.06) 0%, transparent 70%)', pointerEvents: 'none', filter: 'blur(80px)',
      }} />
      <div style={{
        position: 'fixed', bottom: '20%', right: '10%', width: '400px', height: '400px', background: 'radial-gradient(circle, rgba(99, 102, 241, 0.05) 0%, transparent 70%)', pointerEvents: 'none', filter: 'blur(80px)',
      }} />
        ...
        <div className={`new-game-container new-game-container--${gameState}`}>
          ...
          <div className="new-game-container__content" data-tour-id="game-welcome">
            ...
            <div className="new-game-container__betting" data-tour-id="receipts">
              {/* Position Eval first */}
              <div style={{
                background: 'var(--card-bg)', border: '1px solid var(--card-border)', borderRadius: 12, padding: 16, marginBottom: 16,
              }}>
                <div style={{
                  fontSize: 11, textTransform: 'uppercase', letterSpacing: '2px', color: 'var(--text-secondary)', marginBottom: 8,
                }}>Position Eval</div>
                {(() => {
                  ...
                  if (!(sum > 0)) { pWhite = 0.33; pDraw = 0.34; pBlack = 0.33; sum = 1; } else { pWhite /= sum; pDraw /= sum; pBlack /= sum; }
                  if (gameState === 'ended') {
                  ...
                  return (
                    <>
                      <div style={{
                        height: 8, borderRadius: 4, overflow: 'hidden', display: 'flex', background: 'var(--bg-tertiary)', border: '1px solid var(--border-primary)',
                      }}>
                      ...
                      <div style={{
                        display: 'flex', justifyContent: 'space-between', marginTop: 8, fontSize: 11, color: 'var(--text-secondary)',
                      }}>
