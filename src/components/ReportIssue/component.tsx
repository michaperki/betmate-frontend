...
const ReportIssue: React.FC<ReportIssueProps> = ({ isOpen, onClose }) => {
  ...
  return createPortal(
    <div role="dialog" aria-modal="true" aria-labelledby="issue-modal-title" style={{ position: 'fixed', inset: 0, zIndex: 10000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16 }}>
      ...
      <div style={{ position: 'relative', width: 'min(560px, 92vw)', background: 'linear-gradient(180deg, #1a1a24 0%, #12121a 100%)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 16, boxShadow: '0 20px 60px rgba(0,0,0,0.5)', color: '#e8e8e8', overflow: 'hidden' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '14px 16px', borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
          ...
          <button onClick={onClose} aria-label="Close" style={{ background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.12)', width: 28, height: 28, borderRadius: 8, color: '#e8e8e8', cursor: 'pointer' }}>×</button>
        </div>
        <div style={{ padding: 16 }}>
          ...
          <div style={{ display: 'flex', gap: 10, justifyContent: 'flex-end', marginTop: 12 }}>
            <button onClick={onClose} style={{ padding: '8px 12px', borderRadius: 8, background: 'transparent', border: '1px solid rgba(255,255,255,0.2)', color: '#e8e8e8' }}>Cancel</button>
            <button onClick={submit} disabled={submitting || text.trim().length < 3} style={{ padding: '8px 12px', borderRadius: 8, background: 'linear-gradient(135deg, var(--mode-accent) 0%, var(--mode-accent-strong) 100%)', border: 'none', color: '#000', fontWeight: 800, opacity: (submitting || text.trim().length < 3) ? 0.6 : 1 }}>{success ? 'Sent!' : (submitting ? 'Sending…' : 'Send')}</button>
          </div>
        </div>
      </div>
    </div>,
