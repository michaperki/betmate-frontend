...
const Onboarding: React.FC = () => {
  ...
  return (
    <div style={{
      minHeight: '100vh', background: 'linear-gradient(145deg, #0a0a0f 0%, #12121a 50%, #0a0a0f 100%)', fontFamily: '\'JetBrains Mono\',\'SF Mono\',monospace', color: '#e8e8e8', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '40px 20px', position: 'relative', overflow: 'hidden',
    }}>
      <div style={{
        position: 'fixed', top: '10%', left: '20%', width: 600, height: 600, background: 'radial-gradient(circle, rgb(var(--mode-accent-rgb) / 0.08) 0%, transparent 70%)', pointerEvents: 'none', filter: 'blur(80px)',
      }} />
      <div style={{
        position: 'fixed', bottom: '10%', right: '10%', width: 500, height: 500, background: 'radial-gradient(circle, rgba(99,102,241,0.06) 0%, transparent 70%)', pointerEvents: 'none', filter: 'blur(80px)',
      }} />
      <div style={{
        position: 'fixed', top: '15%', left: '8%', fontSize: 120, opacity: 0.03, transform: 'rotate(-15deg)',
      }}>♞</div>
      <div style={{
        position: 'fixed', bottom: '20%', right: '5%', fontSize: 100, opacity: 0.03, transform: 'rotate(10deg)',
      }}>♛</div>
      <div style={{
        position: 'fixed', top: '60%', left: '5%', fontSize: 80, opacity: 0.02, transform: 'rotate(5deg)',
      }}>♜</div>

      <div style={{ width: 680, maxWidth: '100%', zIndex: 1 }}>
        <div style={{
          display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 12, marginBottom: 40,
        }}>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 4 }}>
            <div style={{
              width: 14, height: 14, borderRadius: '50%', background: '#fbbf24',
            }} />
            <div style={{
              width: 14, height: 14, borderRadius: '50%', background: '#f87171',
            }} />
            <div style={{
              width: 14, height: 14, borderRadius: '50%', background: 'var(--mode-accent)',
            }} />
            <div style={{
          ...
          <span style={{
            fontSize: 28, fontWeight: 700, color: 'var(--mode-accent)', letterSpacing: '1px',
          }}>BetMate</span>
        </div>
        ...
        {step > 0 && step < 5 && (
          <div style={{
            ...
            {[1, 2, 3, 4].map((i) => (
              <div key={i} style={{
                width: i <= step ? 32 : 12, height: 4, borderRadius: 2, background: i <= step ? 'var(--mode-accent)' : 'rgba(255,255,255,0.1)', transition: 'all 0.3s ease',
              }} />
            ))}
          </div>
        )}
        ...
        <div style={{
          background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 24, padding: 40, backdropFilter: 'blur(10px)',
        }}>
          {step === 0 && (
            <div style={{ textAlign: 'center' }}>
              <div style={{
                width: 80, height: 80, background: 'linear-gradient(135deg, rgb(var(--mode-accent-rgb) / 0.20) 0%, rgb(var(--mode-accent-rgb) / 0.05) 100%)', borderRadius: 20, display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 24px', fontSize: 40,
              }}>♟️</div>
              <h1 style={{ fontSize: 28, fontWeight: 700, margin: '0 0 12px' }}>Bet on Chess, Live</h1>
              <p style={{
                fontSize: 15, opacity: 0.6, margin: '0 0 32px', lineHeight: 1.6,
              }}>Predict moves, bet on outcomes, and win while watching the world's best players compete.</p>
              {referred && inviteCode && (
                <div style={{ fontSize: 12, color: '#a1a1aa', marginBottom: 12 }}>Invite code applied: <code>{inviteCode}</code></div>
              )}
              <button onClick={() => setStep(1)} style={{
                width: '100%', padding: 16, background: 'linear-gradient(135deg, var(--mode-accent) 0%, var(--mode-accent-strong) 100%)', border: 'none', borderRadius: 12, color: '#000', fontSize: 15, fontWeight: 700, fontFamily: 'inherit', cursor: 'pointer', marginBottom: 16,
              }}>Get Started</button>
          ...
          {step === 1 && (
            <div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                <input placeholder="Username" value={formData.username} onChange={(e) => updateForm('username', e.target.value)} style={{
                  padding: 12, borderRadius: 10, background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.1)', color: '#e8e8e8',
                }} />
                <input placeholder="Email" type="email" value={formData.email} onChange={(e) => updateForm('email', e.target.value)} style={{
                  padding: 12, borderRadius: 10, background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.1)', color: '#e8e8e8',
                }} />
              </div>
              <div style={{
                ...
                <input placeholder="Password" type="password" value={formData.password} onChange={(e) => updateForm('password', e.target.value)} style={{
                  padding: 12, borderRadius: 10, background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.1)', color: '#e8e8e8',
                }} />
                <input placeholder="Confirm Password" type="password" value={formData.confirmPassword} onChange={(e) => updateForm('confirmPassword', e.target.value)} style={{
                  padding: 12, borderRadius: 10, background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.1)', color: '#e8e8e8',
                }} />
              </div>
          ...
          {step === 2 && (
            <div>
              ...
              <div style={{
                ...
                <button onClick={simulateWalletConnect} disabled={walletConnecting} style={{
                ...
                }}>{walletConnecting ? 'Connecting…' : 'Connect'}</button>
              ...
              <div style={{ marginTop: 16, display: 'flex', gap: 12 }}>
                ...
                <button disabled={!canProceed()} onClick={() => setStep(3)} style={{
                  padding: '10px 16px', background: canProceed() ? 'linear-gradient(135deg, var(--mode-accent) 0%, var(--mode-accent-strong) 100%)' : 'rgb(var(--mode-accent-rgb) / 0.15)', border: 'none', borderRadius: 8, color: '#000', fontWeight: 700, cursor: canProceed() ? 'pointer' : 'not-allowed',
                }}>Continue</button>
              </div>
            </div>
          ...
          {step === 5 && (
            <div style={{ textAlign: 'center' }}>
              ...
              <a href={returnTo} style={{
                display: 'inline-block', marginTop: 12, padding: '12px 16px', background: 'linear-gradient(135deg, var(--mode-accent) 0%, var(--mode-accent-strong) 100%)', borderRadius: 8, color: '#000', fontWeight: 700, textDecoration: 'none',
              }}>Go to Dashboard</a>
            </div>
          )}
        </div>
